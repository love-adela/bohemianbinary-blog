...
def populate():
    ...
    for tr in table.find_all('tr'):
        ...
        session.add(db.History(date, open_price, high_price, low_price, close_price, 'BTC'))
    session.commit()
...
def get_data():
    ...
    low_prices = {}
    close_prices = {}
    for row in session.query(db.History).filter(db.History.coin_type == 'BTC').order_by(db.asc(db.History.date)):
