...
def populate():
    session = db.Session()
    response = requests.get('https://coinmarketcap.com/currencies/ethereum/historical-data/?start=20170626&end=20180626')
    ...
    for tr in table.find_all('tr'):
        ...
        session.add(db.History(date, open_price, high_price, low_price, close_price, 'ETH'))
    session.commit()
...
def get_data():
    ...
    low_prices = {}
    close_prices = {}
    for row in session.query(db.History).filter(db.History.coin_type == 'ETH').order_by(db.asc(db.History.date)):
